...
flags.DEFINE_float('alpha_critic',None, 'coffeient for conservative') 
...
def main(_):   #num_samples
  ...
  #      config["sigma"] = sigma
   ...
   # print ("config",config)
    ...
    @jax.jit
    def scan_update(agent, x):
        batch, current_step = x
        agent, info = agent.update(batch, current_step=current_step)
        return agent, info

    #for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
    rng = jax.random.PRNGKey(FLAGS.seed)    
    train_time = 0
    pbar = tqdm.tqdm(range(1, FLAGS.offline_steps + 1), smoothing=0.1, dynamic_ncols=True)
