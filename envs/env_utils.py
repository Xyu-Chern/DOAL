...
def fql_make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5):
    ...
    if 'singletask' in env_name:
        # OGBench.
        env, train_dataset, val_dataset = ogbench.make_env_and_datasets(env_name,dataset_dir=os.environ.get('OGBENCH_DATASET_DIR', os.path.expanduser("$HOME/.ogbench/data")) )
...
def make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5,eval_episodes=1):
    """Make offline RL environment and datasets.
    ...
    if frame_stack is not None:
        env = FrameStackWrapper(env, frame_stack)
        eval_envs = [FrameStackWrapper(eval_env, frame_stack) for eval_env in eval_envs]
    ...
    return env, eval_envs, train_dataset, val_dataset
