...
algo = WandBTD3.create(
    ...
    # TD3特定参数（针对Ant的四足运动任务）
    exploration_noise=0.4,        # 中等探索噪声，平衡稳定性与探索
    target_noise=0.5,             # 中等目标噪声
    target_noise_clip=0.5,        # 中等噪声裁剪
    policy_delay=2,               # 适中的策略延迟
    
    # 并行环境参数
    num_envs=128,
    
    # 优化器参数（基于SAC学习率调整）
    learning_rate=0.00015,        # TD3通常需要稍高的学习率（1.5e-4 vs 1.2e-4）
    ...
    # RL核心参数
    gamma=0.995,                   # 较低的折扣因子，适合蚂蚁的移动任务
    polyak=0.99,                 # 较慢的目标网络更新
    
    # 训练稳定性
    max_grad_norm=1.5,           # 中等梯度裁剪，蚂蚁需要一定灵活性
    
    normalize_observations=True,
    
    # 可选：wandb配置
    # project="td3-brax-ant",
    # name="ant-td3-experiment"
)
