...
class DMFQLAgent(DOALAgent):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        next_actions = self.fql_sample_actions(batch['next_observations'], seed=sample_rng)
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        doal_distill_loss, infos = self.doal_loss(batch, grad_params, rng)

        # Total loss.
        actor_loss = bc_flow_loss + self.config['alpha_actor'] * ( distill_loss + doal_distill_loss ) + q_loss

        # Additional metrics for logging.
        actions = self.fql_sample_actions(batch['observations'], seed=rng)
    ...
    def doal_loss(self, batch, grad_params, rng=None,aux={}):
    ...
    def create(
        ...
        actor_doal_flow_def = ActorVectorField(
            hidden_dims=config['actor_hidden_dims'],
            action_dim=action_dim,
            layer_norm=config['actor_layer_norm'],
            encoder=encoders.get('actor_onestep_doal_flow'),
        )
        ...
        network_info = dict(
           ...
           actor_flow = (actor_doal_flow_def, (ex_observations, ex_actions, ex_times))
        )
        if encoders.get('actor_bc_flow') is not None:
            # Add actor_bc_flow_encoder to ModuleDict to make it separately callable.
            network_info['actor_bc_flow_encoder'] = (encoders.get('actor_bc_flow'), (ex_observations,))
            network_info['actor_flow_encoder'] = (encoders.get('actor_bc_flow'), (ex_observations,))
