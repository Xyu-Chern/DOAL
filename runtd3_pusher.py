...
wandb.init(
    ...
    config={"algo": "TD3", "env": "brax/pusher"}
)

from flax import linen as nn

algo = WandBTD3.create(
    # 环境设置（Pusher需要较长的episode length）
    env="brax/pusher",
    # 注意：ReJax可能需要通过环境包装器设置episode_length
    
    # 网络结构参数
    actor_kwargs={"activation": "tanh"},
    critic_kwargs={"activation": "tanh"},
    
    # TD3特定参数（针对Pusher的物体操控任务）
    exploration_noise=0.3,        # 中等探索噪声，平衡探索与精确控制
    target_noise=0.2,             # 中等目标噪声
    target_noise_clip=0.3,        # 中等噪声裁剪
    policy_delay=2,               # 中等策略延迟，适合复杂操控任务
    
    # 并行环境参数
    num_envs=128,
    
    # 优化器参数（基于SAC学习率调整）
    learning_rate=0.00025,        # 比SAC稍高的学习率（2.5e-4 vs 2.05e-4）
    ...
    # RL核心参数
    gamma=0.98,                   # 稍高的折扣因子，适合稍长序列
    polyak=0.995,                 # 较慢的目标网络更新
    
    # 训练稳定性
    max_grad_norm=1.0,            # 中等梯度裁剪，Pusher需要一定稳定性
    
    normalize_observations=True,
    
    # 可选：wandb配置
    # project="td3-brax-pusher",
    # name="pusher-td3-v1"
)
