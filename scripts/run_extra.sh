...
if [ "$#" -lt 1 ]; then
    echo "Usage: $0 <agent_name> [exp_name] [extra_args...]"
    echo "Example: $0 my_new_agent my_experiment --param_name value"
    exit 1
fi

# Assign command-line arguments to variables for clarity
AGENT_NAME=$1
# Shift arguments so that $@ now refers to exp_name and any additional arguments
shift
...
for env_name in "${env_names[@]}"; do
    echo "Running with Agent: $AGENT_NAME, Env: $env_name "
    python main.py \
