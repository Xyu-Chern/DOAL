...
def compute_actions_for_neighbors_vmap(neighbor_obs, neighbor_data_actions, agent, seed):
    """使用vmap批量计算adjusted和sampled actions"""
    ...
    neighbor_adjusted_actions = compute_adjusted_action(neighbor_obs, neighbor_data_actions)
...
def main(_):
  ...
  #      config["sigma"] = sigma
    ...
    # 使用vmap批量计算adjusted和sampled actions
    neighbor_adjusted_actions, neighbor_sampled_actions = compute_actions_for_neighbors_vmap(
    ...
    for i, (ax, title, color, actions) in enumerate(zip(axes, titles, colors, action_sets)):
        scatter = ax.scatter(actions[:, dim_indices[0]], 
                           actions[:, dim_indices[1]], 
                           alpha=0.7, s=30, color=color, label=title)
        ax.set_title(f'{title} (Top {FLAGS.num_neighbors} neighbors)', fontsize=14)
        ax.set_xlabel(f'Dimension {dim_indices[0]}', fontsize=12)
        ax.set_ylabel(f'Dimension {dim_indices[1]}', fontsize=12)
    ...
    plt.suptitle(f'Action Distribution for Obs with Highest Variance (Obs index: {best_obs_idx})', fontsize=16)
    plt.tight_layout()
    ...
    # 详细统计分析
    print(f"\n=== 详细统计分析 ===")
    for i, (name, actions) in enumerate(zip(['Data', 'Adjusted', 'Sampled'], 
