...
from envs.env_utils import make_env_and_datasets, fql_make_env_and_datasets
from utils.datasets import Dataset, ReplayBuffer
from utils.evaluation import evaluate, flatten, doal_evaluate
...
def main(_):
    ...
    config = FLAGS.agent
    env, eval_env, train_dataset, val_dataset = fql_make_env_and_datasets(FLAGS.env_name, frame_stack=FLAGS.frame_stack)
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if FLAGS.eval_interval != 0 and (i == 1 or i % FLAGS.eval_interval == 0):
            ...
            doal_eval_info, doal_trajs, doal_cur_renders = doal_evaluate(
                agent=agent,
                env=eval_env,
                config=config,
                num_eval_episodes=FLAGS.eval_episodes,
            ...
            for k, v in doal_eval_info.items():
                eval_metrics[f'doal_evaluation/{k}'] = v
