...
class DMeanFlowQL_Agent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
    ...
    @jax.jit
    def get_guided_action(self, action,observation):


        def bc_loss_wrt_q_action(q_action):
            qs = self.network.select('critic')(observation, action)
        ...
        norm_up = norm_mean + self.config["upper_range"] * norm_std

        clipped_g = jnp.where(norm > norm_up,  g * norm_up / norm,   g)
        dx =   (self.config["delta"] / norm_mean ) * clipped_g 
        adjusted_actions = action + dx
        if self.config["clip"]:
    ...
    def meanflow_loss(self, batch, grad_params, rng):
        ...
        actions = batch["guided_actions"] #batch['actions']
        # Meanflow Training
        ...
        mean_flow_loss = jnp.mean(jnp.square(err))
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        actor_loss =  bound_loss * self.config.get('bound_loss_weight', 1) # Add bound loss 
    ...
    def total_loss(self, batch, grad_params, rng, current_step=0):
        ...
        batch["guided_actions"] = self.get_guided_action(batch["actions"],batch["observations"])
        # Critic network
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            use_dynamic_alpha = False, # Boolean flag: True for dynamic, False for cosine
