...
class NDFReRACAgent(flax.struct.PyTreeNode):
    ...
    def dmf_actor_loss(self, batch, grad_params, rng=None,aux={}):
        ...
        if self.config["distill_from_target"]:
            qs = self.network.select('target_critic')(batch['observations'], actions=adjusted_actions)
        else:
            qs = self.network.select('critic')(batch['observations'], actions=adjusted_actions)
        ...
        actor_loss =  jnp.mean(raw_actor_loss) * self.config['alpha_actor']  + n_ensemble_loss

        return actor_loss, {
            "raw_actor_loss":jnp.mean(raw_actor_loss),
            "n_ensemble_loss" : n_ensemble_loss,
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='ndfrebrac',  # Agent name.
            num_ensembles = 2,
