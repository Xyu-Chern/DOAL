...
wandb.init(
    ...
    config={"algo": "TD3", "env": "brax/humanoidstandup"}
)
...
algo = WandBTD3.create(
    ...
    # TD3特定参数（HumanoidStandup的特殊配置）
    exploration_noise=0.8,        # 高探索噪声，鼓励多样化的站立尝试
    target_noise=1.0,             # 非常高的目标噪声
    target_noise_clip=0.9,        # 接近1.0的噪声裁剪
    policy_delay=10,              # 非常大的策略延迟，减少策略更新频率
    
    # 并行环境参数
    num_envs=128,
    
    # 优化器参数
    learning_rate=0.0001538065811598203,  # ~1.538e-4（较低的学习率）
    
    # 缓冲区参数
    buffer_size=1048576,  # 1,048,576
    fill_buffer=8192,
    batch_size=256,
    ...
    # RL核心参数
    gamma=0.99,               # 标准折扣因子
    polyak=0.95,              # 目标网络更新系数
    
    # 训练稳定性（非常严格的梯度控制）
    max_grad_norm=0.2,        # 极小的梯度裁剪阈值
    
    normalize_observations=True,
    
    # 可选：wandb配置
    # project="td3-humanoid-standup",
    # name="humanoid-standup-td3"
)
