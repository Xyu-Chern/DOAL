...
class TrigQLAgent(DOALAgent):
    ...
    def actor_loss(self, batch, grad_params, rng):
     ...
     #   vel =  jnp.cos(t)* z  - jnp.sin(t) * batch['actions']
        ...
        bc_flow_loss = jnp.mean((pred_actions - batch['actions']) ** 2)
    ...
    def sample_actions_simple(
        ...
        for i in range(self.config['flow_steps']):
            t = jnp.full((*n_observations.shape[:-1], 1), (1.0 - i / self.config['flow_steps']) *math.pi / 2)
            vels = self.network.select('actor_flow')(n_observations, actions, t, is_encoded=True)
            s = jnp.full((*n_observations.shape[:-1], 1), (1.0 - (i+1) / self.config['flow_steps']) *math.pi / 2)
